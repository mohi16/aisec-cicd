...
public class SearchService {
    ...
    private void sortResults(List<Note> notes, String sortBy) {
        ...
        switch (value.toLowerCase()) {
            case "title" -> notes.sort(Comparator.comparing(
                    note -> note.getTitle() == null ? "" : note.getTitle().toLowerCase()
            ));
            case "id" -> notes.sort(Comparator.comparing(Note::getId));
            case "updatedat" -> notes.sort(Comparator.comparing(
                    Note::getUpdatedAt,
                    Comparator.nullsLast(Comparator.reverseOrder())
            ));
            default -> notes.sort(Comparator.comparing(
                    Note::getCreatedAt,
                    Comparator.nullsLast(Comparator.reverseOrder())
            ));
        }
